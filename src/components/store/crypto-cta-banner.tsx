...
import { Mail, ArrowRight, Lock, ShieldCheck, Gift, Clock, Loader2, Zap, TrendingUp, Star } from 'lucide-react'
...
const perks = [
  { icon: Zap, label: 'Early access to new products' },
  { icon: Gift, label: '10% off your first order' },
  { icon: TrendingUp, label: 'Exclusive market insights' },
  { icon: Star, label: 'Members-only deals' },
]

export function CryptoCtaBanner() {
    ...
    <section className="relative overflow-hidden py-16 sm:py-24">
      ...
      <div className="absolute inset-0 z-0">
        <video
          ...
          className={`absolute inset-0 size-full object-cover transition-opacity duration-700 ${videoLoaded ? 'opacity-30' : 'opacity-0'}`}
        >
      ...
      {/* Gradient Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-cyan-950/50 via-background/95 to-teal-950/40" />
      ...
      <div className="absolute left-[8%] top-[20%] select-none text-4xl font-bold text-cyan-500/8 crypto-float-1">₿</div>
      <div className="absolute right-[12%] top-[30%] select-none text-3xl font-bold text-teal-500/6 crypto-float-2">Ξ</div>
      <div className="absolute left-[18%] bottom-[25%] select-none text-2xl font-bold text-cyan-400/5 crypto-float-2">◎</div>
      <div className="absolute right-[20%] bottom-[20%] select-none text-3xl font-bold text-teal-400/6 crypto-float-1">₿</div>
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="mb-2 text-3xl sm:text-4xl font-bold text-foreground">
            Stay Ahead in <span className="text-gradient-cyan">Crypto Security</span>
          </h2>
          ...
          <div className="mb-6 flex flex-wrap items-center justify-center gap-3">
            {perks.map((perk) => (
              <div key={perk.label} className="flex items-center gap-1.5 rounded-full border border-border/50 bg-card/60 px-3 py-1.5 text-xs text-muted-foreground backdrop-blur-sm">
                <perk.icon className="size-3 text-cyan-400" />
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {[
                  ...
                  <div
                    key={i}
                    className={`flex size-7 items-center justify-center rounded-full border-2 border-background text-[10px] font-bold text-white ${avatar.color}`}
                  >
                    {avatar.initial}
                  </div>
                ))}
          ...
          {subscribed ? (
            ...
            <form
              ...
              className="flex flex-col gap-3 sm:flex-row max-w-xl mx-auto"
            >
